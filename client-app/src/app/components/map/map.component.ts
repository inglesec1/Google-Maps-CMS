...
export class MapComponent implements OnInit {
  ...
  updateMarkerLocation = (endLocation: google.maps.LatLng, index: number) => {
    this._Map.dragMarker(index, { lat: endLocation.lat(), lng: endLocation.lng() }).subscribe(
      (result) => {
        if (result.success) {
          console.log("Request success");
          ...
          console.log("Request error");
          this.Model.MapMarkers[index].setPosition(new google.maps.LatLng(this.Model.Markers[index].Position.lat, this.Model.Markers[index].Position.lng));
        }
      }
    );
  }
